...
export async function GET(req: NextRequest) {
    ...
    const tokenRes = await fetch(
        `https://api.basescan.org/api?module=account&action=tokentx&address=${address}&startblock=0&endblock=99999999&sort=desc&apikey=${apiKey}`
    )
    ...
    const tokenBalances = await Promise.all(
        Object.values(tokenMap).slice(0, 10).map(async (token: any) => {
            const balRes = await fetch(
                `https://api.basescan.org/api?module=account&action=tokenbalance&contractaddress=${token.address}&address=${address}&tag=latest&apikey=${apiKey}`
            )
